...
# start simulation ----------------------------------------------------------------------
nsim <- 500
sim_pc <- foreach(ii = 1:nsim, .packages = c("pcalg", "parallel", "doSNOW")) %dopar% {
  ...
  for (m in 1:M) {
    # PC algorithm
    pc.est[[m]] <- pc(suffStat, indepTest, alpha, p = p, u2pd = "relaxed", maj.rule = TRUE, solve.confl = TRUE)
